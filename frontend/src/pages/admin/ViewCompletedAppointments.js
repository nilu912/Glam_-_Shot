...
export default class ViewCompletedAppointments extends Component {
    constructor(props) {
        super(props);
        this.state = {
            users: [],
    ...
    async fetchData() {
        try {
            const response = await axios.get(`/api/appointments/completed/${this.state.salon_id}`);
    ...
    render() {
        ...
        <div className="overflow-x-auto ml-[0rem] md:ml-[4rem] md:mr-[4rem] mr-0">
          <div style={{fontWeight:'bolder',fontSize:'2rem',display:'flex',justifyContent:'center'}}>View All Completed Appointment</div>
