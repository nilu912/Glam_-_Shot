...
export default class ViewPendingAppointments extends Component {
    constructor(props) {
      super(props);
      this.state = {
        users: [],
    ...
    async fetchData() {
      try {
        const response = await axios.get(
          `/api/appointments/pending/${this.state.salon_id}`
        );
  ...
  render() {
    ...
    return (
      <div className="overflow-x-auto ml-[0rem] md:ml-[4rem] md:mr-[4rem] mr-0">
        <div style={{fontWeight:'bolder',fontSize:'2rem',display:'flex',justifyContent:'center'}}>View All Pendding Appointment</div>
        <table
          ref={this.tableRef}
